    ...
    public <T> List<T> converterJsonParaEntidades(List<String> jsons, Class<T> entidadeClass, OpenAiChatModel chatModel) {
        ...
        for (String json : jsons) {
            ...
            T entidade = ChatClient.create(chatModel).prompt()
                    .user(u -> u.text("Converta o json a seguir {json} para a entidade: {entidade}. Os atributos da entidade que contem prefixo id, devem ser preenchidos com 0. Os atributos do tipo Date devem ser preenchidos como java.util.Date")
                            .params(params))
                    .call()
